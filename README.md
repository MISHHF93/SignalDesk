...
`computeBusinessCoverage()` aggregates real connection state (`listActiveIntegrationSourceSystems`, `@signaldesk/persistence`) against the catalog into a per-category `none`/`partial`/`connected` signal, surfaced on `/integrations`. It is arithmetic over real data, never a fabricated "strong/limited" label — with one real connector, only the `crm` category can ever show `connected` today.
