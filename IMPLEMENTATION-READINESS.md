...
**Twenty-third pass** (2026-08-27, same day, a fifth fresh sweep — a new
category never run before: injection safety and input validation, per
CLAUDE.md's own standing OWASP-top-10 priority): genuinely clean across
all 5 sub-categories checked, two of them positive "not applicable"
results, not absence-of-evidence assumptions. (1) SQL injection: every
`${...}` interpolation across all 79 files in `packages/persistence/src`
was read — every real query value flows through `$1`/`$2` parameterized
placeholders; the only template-literal interpolations are hardcoded
column-list/`LIMIT` constants the app itself declares, or placeholder-
position arithmetic (`$${params.length}`) with the actual value still
pushed to the params array. (2) XSS: zero `dangerouslySetInnerHTML`
anywhere in `apps/web`; the only two places that build raw HTML strings
(`invite-member.ts`, `email-daily-brief.ts` — the latter the one real
path where connector-sourced business data reaches an HTML email body)
both use the same `escapeHtml` helper. (3) Command injection: a full-repo
grep for `child_process`/`exec`/`spawn` found exactly one file, a local
dev/ops script outside the deployed app, using only array-argument
(non-shell) calls with hardcoded or self-derived args — genuinely not
applicable, confirmed by direct search. (4) Path traversal: the
application performs zero filesystem I/O anywhere (CSV import reads an
uploaded file as an in-memory string; the export route returns JSON, not
a file) — genuinely not applicable, confirmed by direct search, not
assumed from the DB-only architecture. (5) Unsafe deserialization: all 3
real (non-test) `JSON.parse` call sites were read — the QuickBooks
webhook only extracts one narrow id field via a type guard, Claude's
structured-output parsing immediately validates via the caller's own Zod
schema, and the one deviation from Zod-everywhere (an OAuth `id_token`
decode) is disclosed in its own doc comment as intentional: the token
comes from the provider's own endpoint over TLS, never client-supplied,
and is used only as a dedup key, never for an authorization decision. All
8 real-sync connectors confirmed to call a `parseSource*Record` Zod
validator before persistence, with no exception. No code changes this
pass. Test count unchanged: **2,312 tests passing (622 persistence,
live)**.

**Twenty-fourth pass** (2026-08-27, same day, a sixth fresh sweep — a new
category never run before: idempotency and duplicate-side-effect risk on
every real, costly external action, not just the 5 "Safe Action"
write-path files already covered): genuinely clean across all 4
sub-categories checked, with one correctly-reasoned exemption verified
rather than assumed. (1) All 5 `approve-*-action.ts` files were
re-verified end-to-end: a real, atomic `UPDATE ... WHERE outcome IS NULL
RETURNING` claims the approval before any real API call runs, and each
`begin*Send`'s own `ON CONFLICT (organization_id, idempotency_key) DO
NOTHING` (keyed off the stable collaboration id, never a fresh per-request
value) protects the separate "resume an already-approved collaboration"
path — both guards are DB-level, so no client-side double-click/re-render
can reach a real external API twice before either one runs. (2) Of the 5
Stripe-mutating actions, 3 (`start-checkout.ts`/`change-plan.ts`/
`manage-addon.ts`) wrap their critical section in a real
`withAdvisoryLock`; the other 2 (`cancel-subscription.ts`/
`resume-subscription.ts`) genuinely don't need one — verified by reading
the actual Stripe SDK call each makes: both are a pure boolean-field PATCH
on the existing subscription (`cancel_at_period_end`), which Stripe's own
API converges idempotently under concurrent duplicate calls, unlike
`change-plan.ts`'s price update (a real double proration charge) or
`manage-addon.ts`'s item add/remove (a distinct subscription item each
time). (3) `create-internal-task.ts`/`create-goal.ts` both enforce a
required, stable, content-derived idempotency key via a real DB unique
constraint — independent of `useFormStatus`'s pending-disable, which is
present only as a UX nicety, not the actual safety mechanism. (4) The CSV
import's per-row `contentHash` idempotency was directly tested against
the specific bypass scenario asked — re-uploading a file with one row
changed and the rest identical — and holds: unchanged rows correctly
dedupe, the changed row correctly inserts as new. No code changes this
pass. Test count unchanged: **2,312 tests passing (622 persistence,
live)**.

**Twenty-fifth pass** (2026-08-27, same day, a seventh fresh sweep — a
new category never run before: numeric precision and financial-
calculation correctness): unlike the prior five sweeps, this one is not
clean — 1 real, evidenced correctness bug found and fixed, plus 1 real,
minor rounding bug found and fixed. (1) `xero/mapper.ts`'s
`mapXeroInvoiceToSourceInvoiceRecord` fed Xero's date-only `DueDate`
straight to `dueAt` as plain midnight UTC, with no buffer — the exact
over-eager mistake `endOfDateOnlyDayUtc` (`@signaldesk/domain`) already
exists to fix, and that every other date-only-due-field connector
(QuickBooks/Asana/Jira) already routes through. For any UTC-negative
timezone (the US, this app's primary market), that fired
`evaluateOverdueInvoice`'s elapsed-time check up to a full calendar day
before the invoice's real local due date had even begun — concretely, an
invoice due August 30 for a New York business could be reported overdue
starting 7:01pm local time on August 29. Fixed by re-anchoring through
the same helper the sibling connectors use; a real regression test
proves a `dueAt` instant that's still before the invoice's local due
date can no longer occur. (2) `quickbooks/mapper.ts`'s multi-invoice
payment split (a single payment line linking to more than one invoice —
already disclosed as "structurally possible, never observed in real
synced data") rounded each invoice's even share independently
(`Math.round(amountCents / n)`), which can lose up to `n - 1` cents off
the line's real total — $100.00 split across 3 invoices became
3333+3333+3333 = 9999, one cent short. Fixed with a `splitCentsEvenly`
helper that distributes the exact remainder across the first invoices
instead, so the parts always sum back to the real total; a regression
test proves the 3-way split now sums to exactly 10,000 cents. `pnpm
check` re-run clean, `DATABASE_URL` confirmed loaded: **2,314 tests
passing (622 persistence, live)**, typecheck/lint/format/db:check/build
all clean.

**Twenty-sixth pass** (2026-08-27, same day, an eighth fresh sweep — a
new category never run before: concurrency and race conditions beyond
the connector token-refresh races already found and fixed): 1 real,
evidenced race found and fixed across all 8 real-sync connectors'
shared cursor-lookup helper, plus 4 other sub-categories confirmed
genuinely clean by tracing the actual SQL/transaction boundaries.
`listRecentSyncJobsForConnection` (`packages/persistence/src/sync-
jobs.ts`) — the helper all 8 connectors call to compute this run's
`cursorBefore` from the previous run's `cursorAfter` — ordered purely by
`started_at desc` with no status filter, so a still-`running` concurrent
sync (a scheduled trigger landing close to a manual "Sync Now") or a
`failed` row (which always has `cursor_after = null`) could be selected
as the cursor source instead of the last real successful run. For most
connectors this only meant a redundant, harmless full resync; for
QuickBooks/Xero specifically, `cursorBefore` also gates a real
closed/paid-invoice status-transition pass, so a `null` `cursorBefore`
there silently skipped detecting invoices paid since the last real
successful sync — a real "incorrect business information" bug (a paid
invoice keeps showing as open/overdue for at least one sync cycle).
Fixed with a new `onlySucceeded` parameter (default `false`, preserving
`computeConnectorHealth`'s own deliberate need for every status) added
to every one of the 9 cursor-lookup call sites across all 8 connectors.
2 new live-DB regression tests directly prove both failure shapes (a
still-running concurrent job, a failed job) are now correctly excluded.
The other 3 sub-categories checked this pass — rate-limit bucket
atomicity (a single `ON CONFLICT ... DO UPDATE ... RETURNING`, genuinely
serializing concurrent upserts), agent-collaboration/investigation-step
writes (all correctly WHERE-guarded or advisory-lock-protected, with one
latent-but-not-currently-reachable hardening opportunity noted in
`completeAgentCollaboration`'s own missing defense-in-depth guard, not
fixed since no live caller can reach it unlocked), and
`internal_cost_events` (an append-only insert per event, no shared
mutable counter to race on) — all came back genuinely clean. `pnpm
check` re-run clean, `DATABASE_URL` confirmed loaded: **2,316 tests
passing (624 persistence, live)**, typecheck/lint/format/db:check/build
all clean.

**Twenty-seventh pass** (2026-08-27, same day, a ninth fresh sweep — a
new category never run before: empty-state, null-handling, and
boundary-condition correctness across the deterministic Intelligence
Core and connector mappers): 1 significant, systemic real bug found and
fixed across 3 connectors — permanent, silent data loss, not just
delayed catch-up — plus 1 smaller real bug fixed. (1) **HubSpot, Gmail,
and Asana's initial/incremental sync loops all advanced their stored
cursor to the _maximum_ `hs_lastmodifieddate`/`internalDate`/
`modified_at` seen across a page-capped run, assuming the fetch returned
results in that same order — none of the three source APIs guarantee
this** (verified directly against each provider's own current API
reference this pass, not assumed): HubSpot's basic `GET /crm/v3/objects/
deals` list (used only for the initial sync — the incremental path
already correctly used the sorted Search API) has no `sort` parameter at
all; Gmail's `users.messages.list` has none either and is documented/
known to return results newest-first; Asana's `GET /tasks` has no
`sort_by` option on either its initial or incremental path. Once a
page/fetch cap (`MAX_DEAL_PAGES`/`MAX_MESSAGE_BODY_FETCHES`/
`MAX_TASK_PAGES_PER_WORKSPACE`) cut a run short, any record processed
with a newer timestamp than an unprocessed one pushed the cursor past
that unprocessed record's own timestamp — permanently failing the
incremental fetch's forward-only filter on every future run, with zero
error, zero skipped-count signal, nothing. Concretely: an account with
more open deals/messages/tasks than one run's cap could permanently lose
some of them from SignalDesk forever, not just see them delayed. Fixed
per-connector, matched to what each API actually allows: **HubSpot** now
routes the initial sync through the same ascending-sorted Search API
call the incremental path already used (anchored at a beginning-of-time
sentinel instead of a real cursor) — the unordered `fetchHubSpotDeals`
function, now unused anywhere in the app, was removed along with its
now-stale doc-comment claim. **Gmail and Asana have no sorted-fetch
option at all on any path**, so both instead now detect when a run was
truncated by either real cap and, when so, leave `cursorAfter` at
`cursorBefore` unchanged rather than advancing it — the next run safely
re-covers the identical window (idempotent via `ON CONFLICT DO NOTHING`)
and is guaranteed to eventually converge once the backlog drops under
one run's cap, the same "the safe direction for a signal to be wrong"
discipline `endOfDateOnlyDayUtc` (`@signaldesk/domain`) already
established. Both also now log a real, visible warning when this
happens, closing part of a related observability gap this same sweep
also surfaced (a truncated run previously recorded `sync_jobs.status =
'succeeded'` identically to a genuinely complete one, with no
distinguishing signal anywhere). (2) `message-follow-up.ts` displayed a
raw, unformatted many-decimal float (e.g. "76.61666666666667 hours
elapsed") in a card's `observedValue` — the exact bug class already
found and fixed in the sibling `ticket-risk.ts`/`lead-risk.ts` (both
correctly wrap the identical field via `formatHours()`), just never
applied here; fixed to match. 6 new regression tests across the 3
connector sync files, each directly proving both the truncated-run and
normal-completion cursor behavior. The rest of this sweep — all 9
Intelligence Capabilities' empty-candidate-set handling, connector
mapper null/undefined/empty-string/zero handling (including the
specific `??` vs `||` distinction that already-fixed money/count fields
correctly avoid), and a full grep for `||` on any numeric/boolean field
in the Intelligence Core and mappers — came back genuinely clean,
verified directly rather than assumed. `pnpm check` re-run clean,
`DATABASE_URL` confirmed loaded: **2,323 tests passing (624 persistence,
live)**, typecheck/lint/format/db:check/build all clean.

**Twenty-eighth pass** (2026-08-27, same day, a tenth fresh sweep — a new
category never run before: PII/data-lifecycle completeness, checking
whether the "delete my organization" anonymization path and the "export
my data" path actually cover every real tenant table that exists today,
given how many new tables this session's own work has added since those
two functions were originally written): 2 real, evidenced gaps found and
fixed, both matching ADR 0018's own explicit warning that this exact
class of gap would recur as the schema grew. (1)
`anonymize_organization()` (migration 0032, extended by 0054b for
messages/support_tickets and 0064b for customer_email_replies) predates
`organization_invites` (0046) — a customer requesting "erase my data"
would still have `organization_invites.email`, a real third-party email
address (often belonging to someone who never signed up and has no
`users` row of their own to be scrubbed by the function's existing
sole-membership-user logic), sitting in the database indefinitely for
every invite the organization ever sent. Fixed with a new out-of-band
...
completely unused. Fixed by wiring in all three (`listGoals`/
`listOpenInternalTasks`/`listRecentAgentCollaborations`), honestly named
to match each function's own real scope (`openInternalTasks`, not
`internalTasks`; `recentAgentCollaborations`, not `agentCollaborations`)
rather than overclaiming completeness. The 5 "Safe Action" write-path
tables' real send records remain a known, disclosed export gap — no
export-shaped query function exists for any of them yet, a larger
undertaking than wiring in an existing one, noted in the export type's
own doc comment rather than silently left out. 2 new live-DB regression
tests directly prove both fixes. `pnpm check` re-run clean, `DATABASE_URL`
confirmed loaded: **2,325 tests passing (626 persistence, live)**,
typecheck/lint/format/db:check/build all clean.

**Twenty-ninth pass** (2026-08-27, same day, an eleventh fresh sweep — a
new category never run before: connector disconnect/reconnect lifecycle
correctness, chosen because it sits directly adjacent to this session's
own sync-cursor fixes): genuinely clean, traced end-to-end rather than
assumed. The specific mechanism most worth suspecting — disconnecting and
later reconnecting the same external account reuses the same
`integrations.id` (confirmed for all 14 connectors: every
`findOrCreateXxxIntegration` conflicts on `(organization_id,
source_system, external_account_id)`, never bare `id`), so the
reconnect's first sync calls the exact same `listRecentSyncJobsForConnection(...,
onlySucceeded: true)` this session's own Twenty-sixth pass fixed, and
would see `sync_jobs` history from _before_ the disconnect — was traced
through to its actual conclusion: this is the mechanism that _prevents_
data loss across a disconnect/reconnect cycle, not one that causes it.
`disconnect_integration` (migration 0019) never touches `sync_jobs`; it
only deletes the vault-stored token and flips `status`. Since every
connector's incremental fetch is a forward-only, inclusive filter from
the last real cursor, resuming from the pre-disconnect cursor on
reconnect naturally re-covers the entire disconnected window — disconnect
freezes the cursor, it never advances it. Also independently confirmed
clean: vault tokens are genuinely deleted on disconnect (never orphaned
or reusable), no sync path can even run against a `'disconnected'`
integration (`status !== 'active'` gates checked at every trigger — "Sync
Now," webhook resolution, the reconciliation cron), `completeSyncJob`'s
own `where status in ('active', 'degraded')` guard prevents an in-flight
sync from resurrecting a row a concurrent disconnect just deactivated,
and `delete-organization.ts`'s interaction is structurally moot (a new
signup always gets a fresh `organization_id`, so there is no reconnect
path back into an anonymized organization). No code changes this pass.
Test count unchanged: **2,325 tests passing (626 persistence, live)**.

**Thirtieth pass** (2026-08-27, same day, a twelfth fresh sweep — list and
pagination boundary correctness across the app's own read APIs, as
opposed to third-party connector-sync pagination already fixed in the
Twenty-seventh pass): mostly a clean, structural finding rather than a
boundary bug — this app has no page-by-page or cursor-based pagination on
any user-facing read anywhere; every list is a small, fixed-`LIMIT` "top
N" dashboard read with a full refetch-and-replace client pattern
(`useBusinessSnapshot`, `useInvestigationSteps`), so the entire
off-by-one/duplicate-at-boundary/non-unique-cursor bug class cannot occur
by construction, verified by reading every `list*` function in
`packages/persistence/src` (confirmed zero `OFFSET` usages repo-wide) plus
both `app/api` routes that return list data. Did find 2 real unbounded
reads, the one bug class that does apply (missing default limits):
`listOrganizationMembers` (`packages/persistence/src/membership.ts:98`)
had no `LIMIT` at all, unlike every sibling "real set" list in the
package — fixed with a `MAX_ORGANIZATION_MEMBERS = 500` cap, oldest-joined
first, matching the established convention (`listGoals`,
`listOverdueInvoices`). `listConnectorConnections`
(`packages/persistence/src/connector-connection.ts:111`) is also
unbounded, but verified this is deliberate and correct, not a bug: it
doubles as `delete-organization.ts`'s complete revoke/disconnect list
during account deletion, so a cap would leave some integrations silently
still connected — a real vault-secret and tenant-data-lifecycle
correctness regression, worse than the display-only risk it would
"fix." Left unbounded with a doc comment now explaining why, so a future
pass doesn't reintroduce the bug by "fixing" it. `pnpm check` re-run
clean, `DATABASE_URL` confirmed loaded: **2,325 tests passing (626
persistence, live)** — unchanged, since the practical fix (adding a
`LIMIT`) needed no new query-shape test and seeding 500+ live rows to
boundary-test the new cap would be prohibitively slow for this package's
existing live-DB test conventions (no sibling `MAX_*` cap is
boundary-tested against real seeded volume either).

**Thirty-first pass** (2026-08-27, same day, a thirteenth fresh sweep —
session/authentication edge cases: concurrent logins, guest-to-real
account upgrade, and invite/account-linking): found the most consequential
bug of the whole session. The profile page tells a guest (Supabase
anonymous sign-in, ADR 0009) "create an account to keep this workspace" —
but `signUpAction` always called plain `supabase.auth.signUp()`, which
unconditionally mints a brand-new `auth.users` row and provisions a
second, separate organization. The guest's real organization (real
connections, goals, tasks) wasn't deleted, but became permanently
unreachable — a guest session has no credentials, so there was no way
back into it. This directly contradicted the product's own stated
promise and was silent, irreversible data loss on the most common path
a guest would take. Fixed by having `signUpAction` detect an anonymous
session (no invite token present) and call Supabase's documented
anonymous-user-upgrade flow, `updateUser({ email, password })`, instead —
converting the same identity in place rather than minting a new one. A
new migration (`0072c`) closes the two gaps that flow doesn't handle on
its own: (1) `public.users.primary_email`, permanently null for a guest,
now syncs on the real `is_anonymous` transition; (2) critically,
`organizations.is_guest` (which grants full unmetered entitlements,
migration 0070) is now cleared on that same transition — without this,
preserving the guest's org across conversion would have accidentally
opened a permanent free-tier bypass (guest → immediate conversion →
unmetered access forever on a real paying-customer identity). The same
migration also hardens a real, if not-currently-reachable, TOCTOU race
in both invite-redemption functions: the invite-consuming `SELECT` took
no row lock, so two concurrent redemptions of one token could both pass
the pending-status check and both insert a real membership before either
committed — a plain `and status = 'pending'` guard on the later `UPDATE`
(the fix's first draft) would only have protected the invite's own
bookkeeping, not the already-inserted duplicate membership, so the actual
fix adds `for update` to the initial `SELECT` instead, closing the race
at its root. A fifth real gap, unrelated to the migration, was fixed the
same pass: `updatePasswordAction` now calls `signOut({ scope: "others" })`
after a successful password change — a reset's whole point in the
compromise scenario that motivates it is cutting off an attacker's
already-stolen session, which `updateUser()` alone never touched.
Two further real, disclosed gaps were found and deliberately NOT fixed
this pass, since neither is currently live/reachable and both need a
larger, separately-scoped decision: an invited signup that authenticates
via OAuth instead of the password form silently drops its invite token
(currently inert — no OAuth provider is enabled in `NEXT_PUBLIC_ENABLED_OAUTH_PROVIDERS`
in production yet, but would start silently dropping real invites the
moment one is); and a user who already has a SignalDesk account and
receives an invite to a second organization hits a functional dead end
(Supabase's generic "already registered" error, no account-linking
path) — a real product-scope question (does this app ever support
multi-org membership per identity?) rather than a narrow bug fix.
Migration applied to dev first (security-advisor scan: only the standard
baseline anonymous-RLS warnings, nothing new), full check re-run clean
against dev, then approved and applied to production (owner sign-off
both times, matching this session's established two-step process);
production verified clean on `get_advisors` (zero findings, better than
dev) and by directly querying the deployed function source
(`pg_get_functiondef`) confirming both row locks, the guest-flag clear,
the new trigger's installation, and its `EXECUTE` grant restricted to
`identity_provisioner`/`service_role` only. 7 new regression tests (6 on
`auth.test.ts` covering the conversion path, the invite-token exclusion,
and the sign-out-others call; 1 on `invites.test.ts` — deliberately not a
`Promise.all` race, which was empirically confirmed across 5 consecutive
runs to pass whether or not the fix was present against this function's
own fast, single-statement execution profile, so instead directly
verifies the deployed function source contains the lock, mirroring how
0072b's production apply was itself verified). `pnpm check` re-run
clean, `DATABASE_URL` confirmed loaded: **2,332 tests passing (627
persistence, live)**.

**Thirty-second pass** (2026-08-27, same day, a fourteenth fresh sweep —
frontend loading/error-state consistency for the 5 "Safe Action" send
buttons): confirmed the 5 buttons are not 5 separate implementations —
all funnel through one shared component (`AgentRecommendationCard`) and
one shared result shape, so the "4 siblings correct, 1 outlier isn't"
pattern this session kept finding elsewhere structurally cannot occur
here. Pending-state disabling, the "already sent" server re-check
(closing any risk from stale client state), and the reconnect-on-auth-
failure link were all confirmed correct and uniform across all 5. Found
and fixed 2 real gaps, both in the shared component (and its sibling
`DraftActionButton`, which produces the drafts these buttons approve),
affecting all 5 connectors identically: (1) every `startTransition`
callback — the send branch, the `create_internal_task` branch, dismiss,
and the draft button — awaited its Server Action call with no `catch`.
Each action's own internal failures are already turned into a returned
`{ ok: false }` by its own try/catch, but a transport-level failure
(dropped connection, aborted request) rejects the promise instead —
silently: the button just re-enables with no message shown at all,
indistinguishable from the click never registering. Fixed by wrapping
all 4 call sites in a real `catch` that surfaces a genuine error message.
(2) The 5-connector send branch never called `router.refresh()` on a
real (non-`alreadySent`) success, unlike its own sibling
`create_internal_task` branch two lines below it, which already does
this specifically because "Your tasks" is server-rendered from
pre-approval data. Without it, a sibling deterministic card for the same
entity (e.g. `invoice_risk` showing the same finding this reminder just
resolved) could keep showing stale state for up to 45 seconds (the next
poll) or indefinitely for anything outside the polled scope, like
`page.tsx`'s SSR-computed "items need attention" count. Fixed by adding
the same `router.refresh()` call, guarded the same way (only on a
genuinely new send, not a no-op re-click of an already-sent proposal).
Disclosed, not fixed: this component family (`agent-recommendation-
card.tsx`, `draft-action-button.tsx`) has zero automated test coverage,
and — unlike every backend `approve-*-action.ts` file, which all have a
paired `.test.ts` — no React component test infrastructure exists
anywhere in this app at all (`@testing-library/react` isn't a
dependency, no `jsdom`/`happy-dom` test environment is configured). Per
this repository's own UI-testing discipline, this fix is honestly
reported as typecheck/build-verified and manually reasoned through
against JavaScript's own promise/exception semantics, not verified
against a live browser reproduction of an actual dropped-connection
mid-request — bootstrapping real component-test infrastructure is a
larger, separately-scoped undertaking than 2 targeted bug fixes, not
attempted this pass. `pnpm check` re-run clean, `DATABASE_URL` confirmed
loaded: **2,332 tests passing (627 persistence, live)** — unchanged, for
the same reason no new test was added.
