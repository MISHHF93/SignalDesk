...
import { getJiraTokens, storeJiraTokens } from "../src/jira-tokens";
import { getTestPool, seedIntegration, seedOrganization } from "./support";

// Mirrors asana-tokens.test.ts exactly — same provider-neutral Vault
// functions (0019), exercised via Jira's own thin wrapper.
describe.skipIf(!process.env.DATABASE_URL)(
  "jira tokens (live database)",
  () => {
    ...
    it("round-trips a real encrypt/decrypt through Vault", async () => {
      ...
      await storeJiraTokens(pool, org.id, integration.id, {
        accessToken: "test-access-token",
        refreshToken: "test-refresh-token",
        expiresAt,
      });

      const tokens = await getJiraTokens(pool, org.id, integration.id);
    ...
    it("never leaks tokens across tenants", async () => {
      ...
      await storeJiraTokens(pool, orgA.id, integration.id, {
        accessToken: "org-a-secret-token",
        refreshToken: "org-a-secret-refresh",
        expiresAt: new Date(Date.now() + 60 * 60 * 1000),
      });

      const tokens = await getJiraTokens(pool, orgB.id, integration.id);
    ...
    it("rejects storing tokens for an integration outside the caller's tenant", async () => {
      ...
      await expect(
        storeJiraTokens(pool, orgB.id, integration.id, {
