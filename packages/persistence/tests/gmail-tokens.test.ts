...
import { getGmailTokens, storeGmailTokens } from "../src/gmail-tokens";
...
// functions (0019), exercised via Gmail's own thin wrapper.
describe.skipIf(!process.env.DATABASE_URL)(
  "gmail tokens (live database)",
  () => {
    ...
    it("round-trips a real encrypt/decrypt through Vault", async () => {
      ...
      await storeGmailTokens(pool, org.id, integration.id, {
        accessToken: "test-access-token",
        refreshToken: "test-refresh-token",
        expiresAt,
      });

      const tokens = await getGmailTokens(pool, org.id, integration.id);
    ...
    it("never leaks tokens across tenants", async () => {
      ...
      await storeGmailTokens(pool, orgA.id, integration.id, {
        accessToken: "org-a-secret-token",
        refreshToken: "org-a-secret-refresh",
        expiresAt: new Date(Date.now() + 60 * 60 * 1000),
      });

      const tokens = await getGmailTokens(pool, orgB.id, integration.id);
    ...
    it("rejects storing tokens for an integration outside the caller's tenant", async () => {
      ...
      await expect(
        storeGmailTokens(pool, orgB.id, integration.id, {
