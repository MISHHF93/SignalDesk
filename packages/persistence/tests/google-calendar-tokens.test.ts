...
} from "../src/google-calendar-tokens";
import { getTestPool, seedIntegration, seedOrganization } from "./support";

// Mirrors gmail-tokens.test.ts exactly — same provider-neutral Vault
// functions (0019), a genuinely separate OAuth grant even for the same
// Google account (see google-calendar-integration.ts's doc comment).
describe.skipIf(!process.env.DATABASE_URL)(
  "google calendar tokens (live database)",
  () => {
    ...
    it("round-trips a real encrypt/decrypt through Vault", async () => {
      ...
      await storeGoogleCalendarTokens(pool, org.id, integration.id, {
    ...
    it("never leaks tokens across tenants", async () => {
      ...
      await storeGoogleCalendarTokens(pool, orgA.id, integration.id, {
    ...
    it("rejects storing tokens for an integration outside the caller's tenant", async () => {
      ...
      await expect(
        storeGoogleCalendarTokens(pool, orgB.id, integration.id, {
