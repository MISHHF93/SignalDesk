...
import { getZendeskTokens, storeZendeskTokens } from "../src/zendesk-tokens";
import { getTestPool, seedIntegration, seedOrganization } from "./support";

// Mirrors jira-tokens.test.ts exactly — same provider-neutral Vault
// functions (0019), exercised via Zendesk's own thin wrapper.
describe.skipIf(!process.env.DATABASE_URL)(
  "zendesk tokens (live database)",
  () => {
    ...
    it("round-trips a real encrypt/decrypt through Vault", async () => {
      ...
      await storeZendeskTokens(pool, org.id, integration.id, {
        accessToken: "test-access-token",
        refreshToken: "test-refresh-token",
        expiresAt,
      });

      const tokens = await getZendeskTokens(pool, org.id, integration.id);
    ...
    it("never leaks tokens across tenants", async () => {
      ...
      await storeZendeskTokens(pool, orgA.id, integration.id, {
        accessToken: "org-a-secret-token",
        refreshToken: "org-a-secret-refresh",
        expiresAt: new Date(Date.now() + 60 * 60 * 1000),
      });

      const tokens = await getZendeskTokens(pool, orgB.id, integration.id);
    ...
    it("rejects storing tokens for an integration outside the caller's tenant", async () => {
      ...
      await expect(
        storeZendeskTokens(pool, orgB.id, integration.id, {
