...
2. **A pending team invite can be permanently consumed by someone who
...
3. **Stripe webhook's `invoice.payment_failed` handler can write a stale
...
4. **Connector "reauth-required" state doesn't exist** — an expired token
...
5. **Daily-brief email "Sent" status is unverified** — Resend's
   ...
   stake.
6. **Role-aware Attention ranking is unbuilt** — its prerequisite is now
...
7. **No AI-quality regression/evaluation harness** — correctly deferred
...
8. **No per-claim evidence citation from the model** — the reconciler's
...
9. **`/integrations` has absorbed more capability than its original
...
10. **General `SignalFusionEngine` remains unbuilt** — the _specific_
...
2. **Add-on purchase/removal double-submit race** (fixed 2026-08-23, not
   ...
   noted at the time.
3. **QuickBooks webhook drop-with-no-reconciliation** (open, P1) — the
