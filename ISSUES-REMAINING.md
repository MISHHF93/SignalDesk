...
- Date: 2026-08-20 (updated 2026-08-21, 2026-08-22, 2026-08-23)
...
1. **A pending team invite can be permanently consumed by someone who
   never actually owns the invited email** (found 2026-08-23, a deep
   read of `provision_identity_and_organization`, drizzle/0048).
   `handle_new_auth_user()` fires on the `auth.users` INSERT trigger —
   i.e. the instant a signup is submitted, before Supabase's own email
   confirmation link has ever been clicked — and that trigger is what
   checks the invite token, marks the invite `accepted`, and creates the
   membership row, all in one transaction. The invite-token match does
   correctly require `lower(oi.email) = lower(p_primary_email)` (a real,
   working defense — you can't accept someone else's invite by knowing
   the token alone), so an attacker who submits a signup claiming a
   victim's email gains nothing usable: `signUpAction` never returns a
   session until that email is actually confirmed, so they can never log
   in as that account. But the invite is still marked `accepted` and the
   membership row still exists at that point — meaning the token is
   permanently burned, and the real invitee's own later, legitimate
   confirmation click (or a fresh accept attempt) finds no pending invite
   left to use. Bounded and recoverable (an org owner/admin can revoke +
   re-send from `/profile`'s Team panel, same as any other "invite
   didn't land" case), and confirmed this project genuinely requires
   email confirmation (`LAUNCH-BLOCKERS.md` #8) rather than
   auto-confirming, so this is a denial-of-the-real-invitee gap, not an
   account-takeover path. A real fix means moving invite acceptance out
   of the signup-submission trigger and into a real post-confirmation
   hook (e.g. a trigger on `auth.users.email_confirmed_at` transitioning
   from null), which changes `provision_identity_and_organization`'s
   transaction shape (today it creates the user row, the invite
   acceptance or solo-org creation, and the membership all atomically in
   one call) — a real trigger-architecture decision to make deliberately,
   not a same-session database patch to a security-definer function
   already three bug-fix migrations deep (0047-0049).

2. **Stripe webhook's `invoice.payment_failed` handler can write a stale
   `past_due` status over a more current one on out-of-order delivery**
   (found 2026-08-23, a fresh read of `billing/webhooks/stripe/route.ts`).
   `handleInvoicePaymentFailed` unconditionally writes
   `status: "past_due"` from the subscription id alone, never checking
   the subscription's actual current state — unlike
   `customer.subscription.*`'s handler (`syncSubscription`), which maps
   the full, current object Stripe includes in that event. Stripe
   documents webhook delivery order is not guaranteed; if a
   `customer.subscription.updated`/`deleted` event reflecting a more
   final state (a successful retry, a real cancellation) is processed
   before an earlier, now-superseded `invoice.payment_failed` for the
   same subscription, this handler overwrites the correct status back to
   `past_due`. `updateSubscriptionFromStripe` has no version/timestamp
   guard at the SQL layer either (`status = $6`, unconditional), so
   nothing stops the write once it's called. Bounded, not silent-forever:
   the billing reconciliation cron (`api/cron/billing-reconciliation/
route.ts`, this session's own P1 #8 fix) asks Stripe directly for
   every linked subscription's real state and self-heals exactly this
   class of drift on its next scheduled run — the actual customer-facing
   entitlement gap this could cause is a transient window, not a
   permanent one. Not fixed here: the two real candidate fixes both need
   more than a narrow patch — re-fetching the subscription's live state
   inside this handler (an extra Stripe API call on every payment-failure
   event) or dropping `handleInvoicePaymentFailed` entirely on the
   assumption `customer.subscription.updated` always accompanies a real
   status change (true per Stripe's documented behavior, but removing an
   existing independent write path changes this handler's defense-in-
   depth posture against a dropped `customer.subscription.updated`
   delivery specifically, which is a real tradeoff to make deliberately,
   not a same-session guess). The existing test
   ...
   ordering case — a real, currently-uncovered gap in test coverage too.
