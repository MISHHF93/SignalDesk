...
import { disconnectMicrosoftOutlookAction } from "./disconnect-microsoft-outlook";

const mockedGetCurrentOrganization = vi.mocked(getCurrentOrganization);
const mockedCreateDatabasePool = vi.mocked(createDatabasePool);
const mockedGetMicrosoftOutlookIntegrationStatus = vi.mocked(
  getMicrosoftOutlookIntegrationStatus,
);
 ...
 */
describe("disconnectMicrosoftOutlookAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectMicrosoftOutlookAction({
        error: null,
      });
      ...
      expect(mockedGetMicrosoftOutlookIntegrationStatus).not.toHaveBeenCalled();
    },
  );
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectMicrosoftOutlookAction({
        error: null,
      ...
      expect(mockedGetMicrosoftOutlookIntegrationStatus).toHaveBeenCalled();
    },
  );
});
