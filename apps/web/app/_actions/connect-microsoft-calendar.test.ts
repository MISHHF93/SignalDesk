...
import { connectMicrosoftCalendarAction } from "./connect-microsoft-calendar";
 ...
 */
describe("connectMicrosoftCalendarAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectMicrosoftCalendarAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectMicrosoftCalendarAction({
        error: null,
