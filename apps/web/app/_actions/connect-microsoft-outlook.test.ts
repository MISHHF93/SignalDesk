...
import { connectMicrosoftOutlookAction } from "./connect-microsoft-outlook";
 ...
 */
describe("connectMicrosoftOutlookAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectMicrosoftOutlookAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Outlook.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectMicrosoftOutlookAction({
        error: null,
