...
import { disconnectZendeskAction } from "./disconnect-zendesk";
 ...
 */
describe("disconnectZendeskAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectZendeskAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectZendeskAction({ error: null }).catch(
      ...
      expect(mockedGetZendeskIntegrationStatus).toHaveBeenCalled();
    },
  );
});
