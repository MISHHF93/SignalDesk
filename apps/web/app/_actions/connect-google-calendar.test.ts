...
import { connectGoogleCalendarAction } from "./connect-google-calendar";
 ...
 */
describe("connectGoogleCalendarAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectGoogleCalendarAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Google Calendar.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectGoogleCalendarAction({
        error: null,
