...
import { connectJiraAction } from "./connect-jira";
 ...
 */
describe("connectJiraAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectJiraAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Jira.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectJiraAction({ error: null }).catch(
