...
import { connectGmailAction } from "./connect-gmail";
 ...
 */
describe("connectGmailAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectGmailAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Gmail.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectGmailAction({ error: null }).catch(
