...
import { disconnectSalesforceAction } from "./disconnect-salesforce";
 ...
 */
describe("disconnectSalesforceAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectSalesforceAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectSalesforceAction({ error: null }).catch(
      ...
      expect(mockedGetSalesforceIntegrationStatus).toHaveBeenCalled();
    },
  );
});
