...
import { disconnectGoogleCalendarAction } from "./disconnect-google-calendar";

const mockedGetCurrentOrganization = vi.mocked(getCurrentOrganization);
const mockedCreateDatabasePool = vi.mocked(createDatabasePool);
const mockedGetGoogleCalendarIntegrationStatus = vi.mocked(
  getGoogleCalendarIntegrationStatus,
);
 ...
 */
describe("disconnectGoogleCalendarAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectGoogleCalendarAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can manage this connection.",
      });
      expect(mockedCreateDatabasePool).not.toHaveBeenCalled();
      expect(mockedGetGoogleCalendarIntegrationStatus).not.toHaveBeenCalled();
    },
  );
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectGoogleCalendarAction({
        error: null,
      ...
      expect(mockedGetGoogleCalendarIntegrationStatus).toHaveBeenCalled();
    },
  );
});
