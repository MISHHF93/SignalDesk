...
import { connectXeroAction } from "./connect-xero";
 ...
 */
describe("connectXeroAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectXeroAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Xero.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectXeroAction({ error: null }).catch(
