...
import { connectStripeAction } from "./connect-stripe";
 ...
 */
describe("connectStripeAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("STRIPE_CLIENT_ID", "test-client-id");
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectStripeAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Stripe.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectStripeAction({ error: null }).catch(
