...
import { disconnectStripeAction } from "./disconnect-stripe";

const mockedGetCurrentOrganization = vi.mocked(getCurrentOrganization);
const mockedCreateDatabasePool = vi.mocked(createDatabasePool);
const mockedGetStripeIntegrationStatus = vi.mocked(getStripeIntegrationStatus);
 ...
 */
describe("disconnectStripeAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectStripeAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can manage this connection.",
      });
      expect(mockedCreateDatabasePool).not.toHaveBeenCalled();
      expect(mockedGetStripeIntegrationStatus).not.toHaveBeenCalled();
    },
  );
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectStripeAction({ error: null }).catch(
      ...
      expect(mockedGetStripeIntegrationStatus).toHaveBeenCalled();
    },
  );
});
