...
import { disconnectLinearAction } from "./disconnect-linear";

const mockedGetCurrentOrganization = vi.mocked(getCurrentOrganization);
const mockedCreateDatabasePool = vi.mocked(createDatabasePool);
const mockedGetLinearIntegrationStatus = vi.mocked(getLinearIntegrationStatus);
 ...
 */
describe("disconnectLinearAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectLinearAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can manage this connection.",
      });
      expect(mockedCreateDatabasePool).not.toHaveBeenCalled();
      expect(mockedGetLinearIntegrationStatus).not.toHaveBeenCalled();
    },
  );
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectLinearAction({ error: null }).catch(
      ...
      expect(mockedGetLinearIntegrationStatus).toHaveBeenCalled();
    },
  );
});
