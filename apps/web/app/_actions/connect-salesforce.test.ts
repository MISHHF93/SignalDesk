...
import { connectSalesforceAction } from "./connect-salesforce";
 ...
 */
describe("connectSalesforceAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectSalesforceAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Salesforce.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectSalesforceAction({ error: null }).catch(
