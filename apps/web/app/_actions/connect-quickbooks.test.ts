...
import { connectQuickBooksAction } from "./connect-quickbooks";
 ...
 */
describe("connectQuickBooksAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectQuickBooksAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect QuickBooks.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectQuickBooksAction({ error: null }).catch(
