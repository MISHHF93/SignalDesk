...
import { connectSlackAction } from "./connect-slack";
 ...
 */
describe("connectSlackAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectSlackAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Slack.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectSlackAction({ error: null }).catch(
