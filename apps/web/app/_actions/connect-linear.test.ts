...
import { connectLinearAction } from "./connect-linear";
 ...
 */
describe("connectLinearAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("LINEAR_CLIENT_ID", "test-client-id");
    vi.stubEnv("LINEAR_CLIENT_SECRET", "test-client-secret");
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await connectLinearAction({ error: null });

      expect(result).toEqual({
        error: "Only an owner or admin can connect Linear.",
      });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await connectLinearAction({ error: null }).catch(
