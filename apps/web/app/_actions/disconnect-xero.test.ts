...
import { disconnectXeroAction } from "./disconnect-xero";
 ...
 */
describe("disconnectXeroAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectXeroAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectXeroAction({ error: null }).catch(
      ...
      expect(mockedGetXeroIntegrationStatus).toHaveBeenCalled();
    },
  );
});
