...
import { disconnectQuickBooksAction } from "./disconnect-quickbooks";
 ...
 */
describe("disconnectQuickBooksAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectQuickBooksAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectQuickBooksAction({ error: null }).catch(
      ...
      expect(mockedGetQuickBooksIntegrationStatus).toHaveBeenCalled();
    },
  );
});
