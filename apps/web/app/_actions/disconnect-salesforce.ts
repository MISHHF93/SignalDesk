"use server";

import { redirect } from "next/navigation";

import { revokeSalesforceRefreshToken } from "@signaldesk/integrations/salesforce";
import {
  createDatabasePool,
  disconnectSalesforceIntegration,
  getSalesforceIntegrationStatus,
  getSalesforceTokens,
  recordAuditEvent,
} from "@signaldesk/persistence";

import { describeActionError } from "../_lib/describe-action-error";
import { getCurrentOrganization } from "../_lib/session";

export interface DisconnectSalesforceState {
  readonly error: string | null;
}

let pool: ReturnType<typeof createDatabasePool> | undefined;

function getPool() {
  pool ??= createDatabasePool();
  return pool;
}

/**
 * Real disconnect: attempts remote revocation against the org's own
 * `instance_url` (best-effort — see `revokeSalesforceRefreshToken`'s doc
 * comment on why this never blocks the local step), then deletes the
 * Vault-stored tokens and marks the integration disconnected, then records
 * the audit event. Mirrors `disconnectSlackAction`'s exact policy.
 */
export async function disconnectSalesforceAction(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars -- useActionState's action signature requires this parameter
  _prevState: DisconnectSalesforceState,
): Promise<DisconnectSalesforceState> {
  const session = await getCurrentOrganization();

  if (!session) {
    return { error: "Sign in to manage this connection." };
  }

  if (session.role !== "owner" && session.role !== "admin") {
    return { error: "Only an owner or admin can manage this connection." };
  }

  const integration = await getSalesforceIntegrationStatus(
    getPool(),
    session.organizationId,
  );

  if (!integration || integration.status !== "active") {
    return { error: "Salesforce is not currently connected." };
  }

  try {
    const tokens = await getSalesforceTokens(
      getPool(),
      session.organizationId,
      integration.id,
    );

    if (tokens) {
      const revoked = await revokeSalesforceRefreshToken(
        integration.externalAccountId,
        tokens.refreshToken,
      );

      if (!revoked) {
        console.error(
          `Salesforce remote token revocation failed for integration ${integration.id}; proceeding with local disconnect anyway`,
        );
      }
    }

    await disconnectSalesforceIntegration(
      getPool(),
      session.organizationId,
      integration.id,
    );

    await recordAuditEvent(getPool(), session.organizationId, {
      userId: session.userId,
      eventType: "integration.disconnected",
      subjectType: "integration",
      subjectId: integration.id,
      outcome: "succeeded",
      metadata: { sourceSystem: "salesforce" },
    });
  } catch (error) {
    return {
      error: describeActionError(error, "Failed to disconnect Salesforce."),
    };
  }

  // Outside the try/catch deliberately — redirect() throws internally to
  // interrupt rendering, and a catch block above would otherwise treat
  // that as a real failure and swallow the redirect.
  redirect("/integrations/salesforce?salesforce=disconnected");
}
