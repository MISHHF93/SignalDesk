...
import { disconnectHubSpotAction } from "./disconnect-hubspot";
 ...
 */
describe("disconnectHubSpotAction — owner/admin role gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it.each(["member", "viewer"] as const)(
    ...
    async (role) => {
      ...
      const result = await disconnectHubSpotAction({ error: null });
  ...
  it.each(["owner", "admin"] as const)(
    ...
    async (role) => {
      ...
      const outcome = await disconnectHubSpotAction({ error: null }).catch(
      ...
      expect(mockedGetHubSpotIntegrationStatus).toHaveBeenCalled();
    },
  );
});
